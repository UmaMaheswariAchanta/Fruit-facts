...
function showFact(fruit) {
  ...
  document.getElementById("quote-author").textContent = `— ${fruit}`;
  currentFruit = fruit; 
}
...
document.querySelectorAll(".buttons button").forEach(button => {
  button.addEventListener("click", () => {
    const fruit = button.className; // Get class name as fruit key
    showFact(fruit);
  });
});
...
document.querySelector(".new").addEventListener("click", () => {
  if (currentFruit && fruitFacts[currentFruit]) {
    showFact(currentFruit);
  } else {
    alert("Please click on a fruit first!");
  }
});
